...
from D4PG_utilities import compute_channel_gain
"""
Vehicular Network Environments.
"""
# High "channel gain": signal arrives strong and clear at the receiver
# Low "channel gain": signal is weak when it reaches the receiver.
from Environment.utilities import generate_complex_normal_distribution, compute_transmission_rate, \
...
class EdgeSimPyEnv(dm_env.Environment):
    """EdgeSimPy Framework Environment built on the dm_env framework."""
    ...
    def __init__(
        ...
        if envConfig is None:
            self._config = env_config.EdgeSimPyEnvConfig()
            ...
            self._config.action_size, self._config.observation_size, self._config.reward_size, \
                self._config.critic_network_action_size = define_size_of_spaces(
                self._config.vehicle_number_within_edges, self._config.edge_number, self._config.task_assigned_number)
        else:
        ...
        if time_slots is None:
            self._time_slots: timeSlots = timeSlots(
                start=self._config.time_slot_start,  # int: 0
                end=self._config.time_slot_end,
                # int: 299 -- but it should be the ending 'Time Step' that is provided in the monitoring file of first-fit alg?!
                slot_length=self._config.time_slot_length,  # int: 1
            )
        else:
    ...
    def step(self, action: np.ndarray):
        ...
        self._reward, cumulative_reward, average_vehicle_SINR, average_vehicle_intar_interference, average_vehicle_inter_interference, \
            average_vehicle_interference, average_transmision_time, average_wired_transmission_time, average_execution_time, average_service_time, successful_serviced_number, task_offloaded_number, task_required_number = self.compute_reward_with_convex_optimization(
            action)
        ...
        if self._time_slots.is_end():
            ...
            return dm_env.termination(observation=observation,
                                      reward=self._reward), cumulative_reward, average_vehicle_SINR, average_vehicle_intar_interference, average_vehicle_inter_interference, \
        ...
        return dm_env.transition(observation=observation,
                                 reward=self._reward), cumulative_reward, average_vehicle_SINR, average_vehicle_intar_interference, average_vehicle_inter_interference, \
            average_vehicle_interference, average_transmision_time, average_wired_transmission_time, average_execution_time, average_service_time, successful_serviced_number, task_offloaded_number, task_required_number
            ...
            for i in range(int(tasks_number_within_edge)):
                ...
                if self._occuiped:
                    if edge_power - edge_occupied_power <= 0:
                        ...
                        vehicle_edge_transmission_power[vehicle_index][edge_index] = 1 / tasks_number_within_edge * (
                                    edge_power - edge_occupied_power)
                else:
                    vehicle_edge_transmission_power[vehicle_index][
                        edge_index] = 1 / tasks_number_within_edge * edge_power
        ...
        for edge_index in range(self._config.edge_number):
            ...
            for other_edge_index in range(self._config.edge_number):
                if other_edge_index != edge_index:
                    vehicle_index_within_other_edge = self._vehicle_index_within_edges[other_edge_index][
                        self._time_slots.now()]
                    for other_vehicle_index in vehicle_index_within_other_edge:
                        other_channel_condition = self._channel_condition_matrix[other_vehicle_index][edge_index][
                            self._time_slots.now()]
                        inter_interference = np.power(np.absolute(other_channel_condition), 2) * cover_mW_to_W(
                            vehicle_edge_transmission_power[other_vehicle_index][other_edge_index])
            ...
            if vehicle_index_within_edge != []:
                for vehicle_index in vehicle_index_within_edge:
                    channel_condition = self._channel_condition_matrix[vehicle_index][edge_index][
                        self._time_slots.now()]
                    for other_vehicle_index in vehicle_index_within_edge:
                        if other_vehicle_index != vehicle_index:
                            ...
                            if other_channel_condition < channel_condition:
                                vehicle_intar_edge_inference[vehicle_index, -1] += np.power(
                                    np.absolute(other_channel_condition), 2) * cover_mW_to_W(
                                    vehicle_edge_transmission_power[other_vehicle_index][edge_index])
            ...
            for i in range(int(tasks_number_within_edge)):
                ...
                if self._occuiped:
                    if edge_power - edge_occupied_power <= 0:
                        ...
                        vehicle_edge_transmission_power[vehicle_index][edge_index] = vehicle_SINR[vehicle_index, -1] / (
                                    1 + vehicle_SINR[vehicle_index, -1]) / divided_sum * (
                                                                                                 edge_power - edge_occupied_power)
                else:
                    vehicle_edge_transmission_power[vehicle_index][edge_index] = vehicle_SINR[vehicle_index, -1] / (
                                1 + vehicle_SINR[vehicle_index, -1]) / divided_sum * edge_power
    ...
    def compute_reward_with_convex_optimization(
        ...
        for edge_index in range(self._config.edge_number):
            vehicle_index_within_edge = self._vehicle_index_within_edges[edge_index][self._time_slots.now()]
            vehicle_observed_index_within_edge = self._vehicle_observed_index_within_edges[edge_index][
                self._time_slots.now()]
            ...
            task_assignment = np.reshape(task_assignment,
                                         newshape=(self._config.vehicle_number_within_edges, self._config.edge_number))
            ...
            for i in range(int(vehicle_number_within_edge)):
                ...
                if self._vehicle_list.get_vehicle_by_index(vehicle_index).get_requested_task_by_slot_index(
                        self._time_slots.now()) != -1:
                    ...
                    if processing_edge_index != edge_index:
                        ...
                        task_index = self._vehicle_list.get_vehicle_by_index(
                            vehicle_index).get_requested_task_by_slot_index(self._time_slots.now())
                        ...
                        wired_transmission_time = data_size / self._config.wired_transmission_rate * self._config.wired_transmission_discount * \
                                                  the_edge.get_edge_location().get_distance(
                                                      self._edge_list.get_edge_by_index(
                                                          processing_edge_index).get_edge_location())
        ...
        for edge_index in range(self._config.edge_number):
            ...
            for i in range(task_sum):
                ...
                task_index = self._vehicle_list.get_vehicle_by_index(vehicle_index).get_requested_task_by_slot_index(
                    self._time_slots.now())
                data_size = self._task_list.get_task_by_index(task_index).get_data_size()
            ...
            new_task_computation_resource_allocation = (task_computation_resource_allocation) / np.sum(
                (task_computation_resource_allocation))
            ...
            for i in range(task_sum):
                ...
                if self._occuiped:
                    if edge_computing_speed - edge_occupied_computing_speed <= 0:
                        ...
                        vehicle_edge_computation_resources[vehicle_index][edge_index] = \
                        new_task_computation_resource_allocation[i] * (
                                    edge_computing_speed - edge_occupied_computing_speed)
                else:
                    vehicle_edge_computation_resources[vehicle_index][edge_index] = \
                    new_task_computation_resource_allocation[i] * edge_computing_speed
                task_index = self._vehicle_list.get_vehicle_by_index(vehicle_index).get_requested_task_by_slot_index(
                    self._time_slots.now())
                data_size = self._task_list.get_task_by_index(task_index).get_data_size()
                computation_cycles = self._task_list.get_task_by_index(task_index).get_computation_cycles()
                if vehicle_edge_computation_resources[vehicle_index][edge_index] != 0:
                    if float(data_size * computation_cycles / vehicle_edge_computation_resources[vehicle_index][
                        edge_index]) < punished_time:
                        vehicle_execution_time[vehicle_index, -1] = float(
                            data_size * computation_cycles / vehicle_edge_computation_resources[vehicle_index][
                                edge_index])
                    else:
                        ...
                        if self._occuiped and occupied_time > 0:
                            ...
                            if end_time < self._config.time_slot_number:
                                for i in range(start_time, end_time):
                                    self._occupied_computing_resources[edge_index][i] += \
                                    vehicle_edge_computation_resources[vehicle_index][edge_index]
                            else:
                ...
                for vehicle_index in vehicle_index_within_edge:
                    for e in range(self._config.edge_number):
                        ...
                        for other_edge_index in range(self._config.edge_number):
                            if e == other_edge_index:
                                ...
                                vehicle_inter_edge_inference[vehicle_index, other_edge_index] += \
                                edge_inter_interference[e]
                    channel_condition = self._channel_condition_matrix[vehicle_index][edge_index][
                        self._time_slots.now()]
                    for other_vehicle_index in vehicle_index_within_edge:
                        if other_vehicle_index != vehicle_index:
                    ...
                    for e in range(self._config.edge_number):
                        if e == edge_index:
                            ...
                            vehicle_intar_edge_inference[vehicle_index, e] = vehicle_intar_edge_inference[
                                vehicle_index, -1]
                    ...
                    if self._occuiped:
                        if transmission_rate != 0:
                            ...
                            if self._occuiped and occupied_time > 0:
                                ...
                                if end_time < self._config.time_slot_number:
                                    for i in range(start_time, end_time):
                                        self._occupied_power[edge_index][i] += \
                                        vehicle_edge_transmission_power[vehicle_index][edge_index]
                                else:
                                    for i in range(start_time, int(self._config.time_slot_number)):
                                        self._occupied_power[edge_index][i] += \
                                        vehicle_edge_transmission_power[vehicle_index][edge_index]
        ...
        for edge_index in range(self._config.edge_number):
            if self._vehicle_index_within_edges[edge_index][self._time_slots.now()] != []:
                for vehicle_index in self._vehicle_index_within_edges[edge_index][self._time_slots.now()]:
                    ...
                    task_service_time = vehicle_transmission_time[vehicle_index, -1] + vehicle_wired_transmission_time[
                        vehicle_index, -1] + vehicle_execution_time[vehicle_index, -1]
                    ...
                    for e in range(self._config.edge_number):
                        if e != edge_index:
                            ...
                            task_service_time = vehicle_transmission_time[vehicle_index, e] + \
        ...
        for j in range(self._config.edge_number):
            ...
            for i in range(vehicle_number_in_edge):
                try:
                    ...
                    task_index = self._vehicle_list.get_vehicle_by_index(
                        vehicle_index=vehicle_index).get_requested_task_by_slot_index(slot_index=self._time_slots.now())
                    ...
                    if task_index == -1:
                        ...
                        index += 1
                        observation[j][index] = float((data_size - self._config.task_minimum_data_size) / (
                                    self._config.task_maximum_data_size - self._config.task_minimum_data_size))
                        index += 1
                        ...
                        # index += 1
                        observation[j][index] = float((delay_threshold - self._config.task_minimum_delay_thresholds) / (
                                    self._config.task_maximum_delay_thresholds - self._config.task_minimum_delay_thresholds))
                        index += 1
                except IndexError:
                    pass
            # print("index 1: ", index)
            ...
            for i in range(self._config.edge_number):
                ...
                observation[j][index] = float((edge_compuation_speed - self._config.edge_minimum_computing_cycles) / (
                            self._config.edge_maximum_computing_cycles - self._config.edge_minimum_computing_cycles))
                index += 1
            # print("index 2: ", index)
            if self._occuiped and not self._for_mad5pg:
                for i in range(self._config.edge_number):
                    observation[j][index] = self._occupied_power[i][self._time_slots.now()] / (
                                1.01 * self._config.edge_power)
                    index += 1
                    observation[j][index] = self._occupied_computing_resources[i][self._time_slots.now()] / (
                                1.01 * self._config.edge_maximum_computing_cycles)
                    index += 1
...
def make_environment_spec(environment: EdgeSimPyEnv) -> EnvironmentSpec:
...
def init_distance_matrix_and_radio_coverage_matrix(
    ...
    channel_condition_matrix = [
        [[[] for _ in range(env_config.time_slot_number)] for _ in range(env_config.edge_number)] for _ in
        range(env_config.vehicle_number)]
    """Get the radio coverage information of each edge node."""
    vehicle_index_within_edges = [[[] for __ in range(env_config.time_slot_number)] for _ in
                                  range(env_config.edge_number)]
    vehicle_observed_index_within_edges = [[[] for __ in range(env_config.time_slot_number)] for _ in
                                           range(env_config.edge_number)]
    for i in range(env_config.vehicle_number):
        for j in range(env_config.edge_number):
            for k in range(env_config.time_slot_number):
                distance = vehicle_list.get_vehicle_by_index(i).get_distance_between_edge(k,
                                                                                          edge_list.get_edge_by_index(
                                                                                              j).get_edge_location())
