...
def my_rl_in_edgesimpy(parameters):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            if num_likely_missed_deadline >= (2 * (len(Service.all()) * len(EdgeServer.all()))) and (count_ones < (0.5 * (len(Service.all())))):
                truncated = True
            if num_likely_missed_deadline >= (3 * (len(Service.all()) * len(EdgeServer.all()))) and (count_ones < (0.6 * (len(Service.all())))):
                truncated = True
            elif num_likely_missed_deadline >= (4 * (len(Service.all()) * len(EdgeServer.all()))) and (count_ones < (0.7 * (len(Service.all())))):
                truncated = True
            elif num_likely_missed_deadline >= (5 * (len(Service.all()) * len(EdgeServer.all()))) and (count_ones < (0.8 * (len(Service.all())))):
                truncated = True
            elif num_likely_missed_deadline >= (6 * (len(Service.all()) * len(EdgeServer.all()))) and (count_ones < (0.9 * (len(Service.all())))):
                truncated = True
            elif num_likely_missed_deadline >= (7 * (len(Service.all()) * len(EdgeServer.all()))):
                truncated = True
            else:
                truncated = False
